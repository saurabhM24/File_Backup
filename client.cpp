...
int send_file(int sockfd, const char *file_name);

std::vector<string> traverse_dir(string dir_name);
...
int main() {
    ...
    */
	...
	std::vector<string> final_file_list = traverse_dir(".");
	printf("Length of filelist: [%d\n]", final_file_list.size());
...
std::vector<string> traverse_dir(string dir_name) {
	...
	while ((ent = readdir(dir)) != NULL) {
			...
			if (!strcmp(".", ent->d_name) || !strcmp("..", ent->d_name)) {
					...
					sprintf(entry_name, "%s/%s", dir_name.c_str(), ent->d_name);
